...
@query
def create_item_revision(fire, user_id, item_id, name, description, quantity, unit):
...
@query
def create_item_comment_revision(fire, editing_user_id, item_comment_id, text):
    now = datetime.now(timezone.utc)
    return fire(editing_user_id, now, item_comment_id, text)["lastrowid"]
